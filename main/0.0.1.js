const HELP="\u200b".repeat(1000)+["/help","/날씨","/디지털시계","/코로나","/가위바위보 [가위|바위|보]","/계산 {식}","/암호화 {문자}","/복호화 {암호화된 문자}","/따라하기 {따라할 말}","/업다운 시작","/업다운 {숫자}","/시간표검색 {학교명}","/시간표검색 {학교코드}","/출석","/한강","/주사위","/채팅수","/가르치기 {가르칠 말}@{이렇게 답하기}","@@{가르친 말}"].join("\n");
