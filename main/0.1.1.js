const HELP="\u200b".repeat(1000)+["/help","/날씨","/시계","/디지털시계","/코로나","/가위바위보 [가위|바위|보]","/계산 {식}","/암호화 {문자}","/복호화 {암호화된 문자}","/따라하기 {따라할 말}","/업다운 시작","/업다운 {숫자}","/시간표검색 {학교명}","/시간표검색 {학교코드}","/출석","/한강","/주사위","/채팅수","/가르치기 {가르칠 말}@{이렇게 답하기}","@@{가르친 말}","/배터리","/한영 {변환할말}"].join("\n");
        ...
        default:
            switch(msg.split(" ")[0]){
                ...
                case "/계산":
                    ...
                    break;
                case "/암호화":
                    replier.reply(amhohwa(msg.replace("/암호화","").trim()));
                    break;
                case "/복호화":
                    replier.reply(bokhohwa(msg.replace("/복호화","").trim()));
                    break;
                case "/따라하기":
                ...
                case "/가르치기":
                    ...
                    break;
                case "/한영":
                    replier.reply(hanyeong(msg.replace("/한영","").trim()));
                    break;
            }
    }
}
